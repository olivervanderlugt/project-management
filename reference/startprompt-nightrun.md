...
to, and this is what tells it. See decision `0005`.
...
1. Fetch first. The default branch moves every night, and other sessions push
   to it during the day — read the current tree, not the one you woke up with.
2. Sweep for residue. Any task on `doing` with no session actually on it is the
...
3. Count open overnight PRs. Three or more: do not build tonight. Spend the run
   sharpening `inbox` tasks into `ready` ones instead, and say so in the log.
4. Otherwise pick exactly one task: highest `ready`, oldest first. Set it to
   `doing`, fill in `branch:`, and write the night-log's opening block BEFORE
   the first expensive step. Trail before work, not after.
5. Build it on night/<task-slug>, off the project's default branch. Commit after
   every self-contained step — a usage-limit cutoff gives no warning and takes
   everything uncommitted with it.
6. Check it adversarially before you believe it. If you have the Agent tool, use
...
7. Tests green or you do not push. Red means: write down exactly what broke, set
   the task to `blocked`, stop.
8. Merge your own PR once the check passed and CI is green — rule 3, decided
   2026-08-10. The checker's pass IS the review gate; it does not additionally
   wait for Ollie. Leave it open only if GitHub itself refuses the merge, and
   then say in the log what it refused on. Then stop: do not start a second task
   because there is budget left.
9. Close out: task status honest (`done`, `blocked` or `split`), dashboard
