...
**Kloon-check — vertakt van de echte keten, niet van de stale default.** De
sessie startte weer op een door de omgeving toegewezen branch die zelf al
stale bleek (dezelfde oude 2026-08-20-commit als vorige nachten). `git fetch`
bevestigt: de officiële default (`claude/hangar-project-setup-kvhcad`) staat
nog steeds op `861f2c3` (Night 2026-08-20), nu 32 nachten achter de echte
keten. Vertakt van `claude/night-2026-09-21` (de tip), niet van de
toegewezen branch en niet van de stale default.
...
Drie blijft drie: over de grens van regel 5, dus geen taak gebouwd vanavond.
Dit is de 32e nacht op rij (2026-08-21 t/m vanavond) op dezelfde drie PR's.

**Stap 3 — `hangar-pr-plafond-kwijt` herverifieerd: geen verandering, 17
nachten stil, oudste nog niet recent herverifieerde inbox-taak.** Alle drie
vaststaande punten vers bij de bron nagegaan: `claude/charming-fermat-kdilid`
bestaat nog steeds zonder PR, `decisions/` bevat nog geen `0007` of ander
besluit dat dit oplost (het `0004`-slot is inmiddels bezet door een ander,
wél gemergd besluit), en de drie PR's die regel 5 vanavond weer laten
gelden zijn dezelfde drie zonder `APPROVED`-review — het besluit had vanavond
zelf dus nog steeds geen ander resultaat gegeven, precies zoals op 09-05 al
vastgesteld. Wel vers geteld: de wrapup-stapel in `project-management` staat
nu op 32 open PR's (was 22 op 09-06, 30 op 09-21). Geen nieuwe bevinding, dus
geen andere uitkomst dan `inbox`, wachtend op Ollie. Volledig uitgeschreven in
de taak zelf (sectie "Herverifieerd 2026-09-22"). `planning/now.md`
bijgewerkt met de verse tellingen.
...
1/5: het plafond geldt); de overige governance-taken
(`hangar-wrapup-pr-pileup`, `nachtrun-loopt-vast-op-een-taak`,
`hangar-daysession-branches-onzichtbaar`, `hangar-in-de-browser`) met rust
gelaten — allemaal wachten op een procesbeslissing van Ollie, geen enkele
deze nacht opnieuw onderzocht; geen wees-PR's of -branches aangeraakt. Wel:
gezien dit nu de 32e opeenvolgende nacht zonder bouw is en Ollie's eigen
plafond-fix al 6 weken ongebruikt klaarligt, is dit buiten de git-trail om
ook als notificatie naar Ollie gestuurd — een maand stilstand die alleen in
een 32e PR blijft staan, bereikt hem anders mogelijk niet.
...
(`claude/night-2026-09-11`, met alle voorgaande nachten er al in verwerkt),
...
- `percentile#1` — `mergeable_state: dirty`, `created_at` = `updated_at`
  (2026-08-07). Ongewijzigd, echt conflict tegen `main`.
- `learning-website#3` — `mergeable_state: clean`, `created_at` = `updated_at`.
  Ongewijzigd, staat nog steeds klaar om te mergen; niemand heeft dat gedaan.
- `project-management#13` (`hangar-stale-clone-guard`) — `mergeable_state:
  dirty`, `status: blocked`, `created_at` = `updated_at`. Ongewijzigd, checker
  hield `ship: false` op het live-hook-verificatiepunt.
...
**Stap 3 — `hangar-in-de-browser` opnieuw gecontroleerd, geen verandering
gevonden.** Oudste nog niet recent herverifieerde inbox-taak (laatst
inhoudelijk aangescherpt 2026-09-02, tien nachten geleden — de zes andere
inbox-taken zijn allemaal binnen de laatste zes nachten gecheckt,
`quizzly-slide-designer-bouwen` bijvoorbeeld gisteren). Rechtstreeks bij
GitHub gecontroleerd, niet aangenomen uit `planning/now.md`:
- `search_issues` op `label:capture` in deze repo → nog steeds 0 resultaten.
  Niemand heeft het Vangen-formulier geprobeerd.
- De default branch heeft nog steeds geen enkele `github-actions[bot]`-commit
  ná `861f2c3` (08-20) — de Capture-workflow heeft nooit gedraaid.
- `decisions/` bevat nog geen besluit dat 0001 (stap 4's blokkade) vervangt.

Niets vooruitbewogen. **Blijft `inbox`**, exact dezelfde open vraag aan Ollie
als op 09-02: probeer het Vangen-formulier zelf vanaf je telefoon. Zelf geen
test-issue aangemaakt — dat zou een echte `github-actions[bot]`-commit
rechtstreeks op de default veroorzaken, los van deze nacht se eigen
PR-stroom, dus buiten de grens. Geen ander inbox-taak dit keer onderzocht
(regel 2: één taak per nacht).

**De wrapup-PR-stapel is opnieuw gegroeid.** Rechtstreeks geteld:
`project-management#15`–`#35`, 21 open stap-4-PR's, plus deze nacht's eigen
PR wordt de 22e. Nog steeds ongeveer één per nacht, zoals
`hangar-wrapup-pr-pileup` voorspelde. Niet zelf gemergd of opgeruimd — dat
blijft Ollie's procesbeslissing. `hangar-pr-plafond-kwijt` (Ollie's eigen,
al geaccepteerde besluit `0004-nachtrun-pr-plafond-verhoogd`, sinds 2026-08-11
nooit doorgevoerd) blijft om dezelfde reden onopgelost en zelf ongewijzigd
...
`hangar-daysession-branches-onzichtbaar`, laatst 09-05 herverifieerd, niet de
taak die vanavond aan de beurt was); geen test-issue aangemaakt op het
Vangen-formulier (zou een ongevraagde commit op de default veroorzaken).
...
**Herstel-noot (toegevoegd 2026-08-31):** deze entry en de nacht-van-08-29-
entry hieronder stonden allebei alleen op hun eigen, nooit-gemergede
wrap-up-branch — allebei vertakt vanaf dezelfde stale default (`861f2c3`), dus
geen van beide zag de andere. De nacht van 08-31 heeft ze samengevoegd (zie
die entry) en dat is precies waarom er hier nu twee aparte 08-29/08-30-
secties naast elkaar staan in plaats van dat 08-30 op 08-29 voortbouwde.
