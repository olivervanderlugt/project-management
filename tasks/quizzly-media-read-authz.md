...
**Dit besluit stond 8 nachten onzichtbaar op de default branch.** Ollie nam het
2026-08-20 15:49 UTC op een branch (`claude/hangar-nightrun-status-hsr0iy`)
zonder PR. Geen enkele nachtrun tussen 08-21 en 08-28 zag hem — elke nacht las
dit bestand nog als `status: inbox` met de oude twee-opties-vraag, en de nacht
van 08-28 herverifieerde zelfs braaf de route-code tegen die stale versie
(zie hieronder) zonder te weten dat de vraag al beantwoord was. Pas de nacht
van 08-29, bij het kiezen van werk, viel de branch op en is hij teruggehaald.
Zie `tasks/hangar-daysession-branches-onzichtbaar.md` voor het bredere
patroon — dit is niet hetzelfde euvel als `hangar-wrapup-pr-pileup` (dat gaat
over de nachtrun se eigen PR's, dit over een dagsessie-branch zonder PR).

**Herchecked 2026-08-28 (nachtrun, stap 3), tegen de actuele code — nog steeds
geldig na het terughalen van Ollie's besluit.** `GET /api/media/[key]/route.ts`
op `origin/main` (na PR #7, laatste relevante merge) is woord-voor-woord
ongewijzigd sinds 2026-08-14: nog steeds geen authz, dezelfde onderbouwing in
de route-header. Geen sessie- of spel-token-infrastructuur is er sindsdien
bijgekomen die optie 2 (sessiegebonden lezen) goedkoper zou maken dan toen —
dus de aanpak hierboven (HMAC-leestoken of het bestaande spelertoken
persisteren) staat nog overeind. Deze verificatie gold destijds de verkeerde
statusregel (`inbox`); de onderliggende codefeiten kloppen nog.
