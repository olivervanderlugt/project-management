...
- Dit is de fysieke uitwerking van wat `hangar-in-de-browser` stap 1 wilde
  ...
  routering).
- Besluit hierover: `decisions/0006-altijd-aan-kastje-thuis.md` (proposed).
  Het gaat op `accepted` zodra het eerste bericht als taak op het bord staat.

## Teruggevonden 2026-08-30 (nachtrun, stap 3)

Dit bestand en het bijbehorende besluit zijn geschreven op 2026-08-06/07 op
branch `claude/pi-openclaw-hangar-plan-n706uo`, maar er is nooit een PR voor
geopend — onvindbaar via `list_pull_requests`, onzichtbaar voor `git status`
op elke latere kloon. 24 dagen lang heeft geen enkele sessie of nachtrun dit
gezien: exact het patroon uit `hangar-daysession-branches-onzichtbaar.md`,
alleen ouder. Vanavond teruggehaald met `git show <branch>:<pad>`, prijzen en
route-status **niet** herverifieerd (dat is aan Ollie of aan wie dit oppakt —
de Max-abonnementsroute-tijdlijn in dit document was al instabiel toen het
geschreven werd). Het besluitnummer is hernoemd van `0004` naar `0006`: `0004`
en `0005` waren inmiddels aan andere besluiten vergeven op de echte default
branch. Status ongewijzigd overgenomen (`ready` / `proposed`) — dit is
capture-herstel, geen inhoudelijke beoordeling.
