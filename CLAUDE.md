...
8. **Trail before work, not after.** Set the task to `doing` and write the
...
9. **Never leave `doing` behind.** A task found `doing` at the start of a run,
...
10. **On limit pressure, downgrade.** If a usage-limit error appears mid-run,
